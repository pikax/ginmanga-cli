...
var regSupported = new RegExp("^" + supported.join("|") + "$", "i");
var site = gin.mangahere;
var ensureDirFile = function ensureDirFile(path) {
...
var downloadAndSave = function downloadAndSave(src, path, bar) {
    return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    return [4 /*yield*/, ensureDirFile(path)];
                case 1:
                    _a.sent();
                    return [2 /*return*/, new Promise(function (resolve, reject) {
                        var stream = fs_extra_1.createWriteStream(path);
                        hyperquest(src).pipe(stream).on("finish", function () {
                            bar.tick();
                            resolve(true);
                        }).on("error", reject);
                    })];
...
program.version(pjson.version).usage("-U mangafox").option("-U, --site [site]", "change current web source default mangahere");
program.command("download <manga> <chap>").alias("d").description("downloads to a specific folder").option("-m, --manga <manga>", "Manga name").option("-c, --chapter <chap>", "chapter").option("-p, --path [path]", "set the path where it downloads", 'download').option("-U, --use [site]", "change current web source default mangahere").action(function (manga, chap, cmd) {
    return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    ...
                    tkImgs = Array();
                    _loop_1 = function _loop_1(c) {
                        var folder = path_1.resolve(process.cwd(), path, site.config.name, manga, c);
                        var imgs_1 = site.images(manga, +c).then(function (x) {
                            bar.tick();
                            return x;
                        }).then(function (x) {
                            return {
                ...
                case 1:
                    ...
                    _loop_2 = function _loop_2(c) {
                        var im = c.imgs.map(function (i) {
                            return i.then(function (x) {
                    ...
                    images = imgs.map(function (p) {
                        return p.then(function (x) {
                            return downloadAndSave(x.src, x.path, bar);
                        });
                    });
